...
const Page = () => {
  ...
  useEffect(() => {
    if (fetchedMessages) {
      setMessages(fetchedMessages); // Update local state with fetched messages
    }
  }, [fetchedMessages]);
  ...
  return (
    <main className="flex flex-col h-screen max-h-screen overflow-hidden">
      <header className="border-b border-zinc-800 p-4 flex items-center justify-between bg-zinc-900/30">
        ...
        <button
          className="text-xs bg-zinc-800 hover:bg-red-600 px-3 py-1.5 rounded text-zinc-40 hover:text-white font-bold transition-all group flex items-center gap-2 disabled:opacity-50"
          onClick={async () => {
            try {
              const res = await fetch(`/api/message?roomId=${roomId}`, {
                method: "DELETE",
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 scrollbar-thin">
        {messages?.length === 0 && (
          <div className="flex items-center justify-center h-full">
            <span className=" text-white-500 animate-pulse">{"</blank>"}</span>
          </div>
        )}
